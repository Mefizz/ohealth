        ...
        foreach ($plans as $rawFhir) {
            ...
            */
            ...
            DB::transaction(function () use ($person, $rawFhir, $activityRepo) {
                $categoryData = null;
                if (isset($rawFhir['category'])) {
                    if (is_array($rawFhir['category'])) {
                        if (isset($rawFhir['category'][0])) {
                            $categoryData = $rawFhir['category'][0];
                        } else {
                            $categoryData = $rawFhir['category'];
                        }
                    }
                }
                ...
                $categoryCode = $categoryData['coding'][0]['code'] ?? null;
                ...
                $encounterId = null;
                if (isset($rawFhir['encounter']['identifier']['value'])) {
                    $encounterId = \App\Models\MedicalEvents\Sql\Encounter::where('uuid', $rawFhir['encounter']['identifier']['value'])->value('id');
                }
                ...
                if (isset($rawFhir['instantiates_protocol'])) {
                    if (is_array($rawFhir['instantiates_protocol'])) {
                        $clinicalProtocol = $rawFhir['instantiates_protocol'][0]['display'] ?? ($rawFhir['instantiates_protocol'][0] ?? null);
                    } else {
                        $clinicalProtocol = $rawFhir['instantiates_protocol'];
                    }
                }

                $context = $rawFhir['context']['identifier']['value'] ?? ($rawFhir['context'] ?? null);
                $requisition = $rawFhir['requisition'] ?? null;
                $informWith = $rawFhir['inform_with'] ?? null;
                ...
                if ($carePlan) {
                    $carePlan->update([
                        ...
                        'clinical_protocol' => $clinicalProtocol,
                        'context' => $context,
