...
trait ManagesEncounterReferrals
{
    ...
    public function openEncounterReferralDrawer(): void
    {
        ...
        foreach ($this->encounterReferralPrograms as $program) {
            if (mb_stripos($program['name'], 'медичних гарантій') !== false || mb_stripos($program['name'], 'пмг') !== false) {
                $defaultProgramId = $program['id'];
                break;
            }
        }
        if ($defaultProgramId === '' && !empty($this->encounterReferralPrograms[0]['id'])) {
            $defaultProgramId = $this->encounterReferralPrograms[0]['id'];
        }
