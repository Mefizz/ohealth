...
class CompositionLifecycleService
{
     ...
     *
     * createComposition (API-006-009-0003) accepts the unsigned compositionRequest JSON
     * body — the same shape Confluence documents for МВН / МВТН. The author's KEP is
     * applied later via {@see sign()}, which is the only create-flow endpoint that wraps
     * content in `{data: <PKCS#7>}`. Sending a signature wrapper here made EMAL try to
     * deserialize `{data: "..."}` as CreateCompositionPayload and fail on ["type"].
     *
     * @param  array<string, mixed>  $compositionRequest  Mapper payload (compositionRequest).
     * @return array{id: string|null, eta: string|null, status: string|null}
     */
    public function create(array $compositionRequest): array
    {
        return $this->jobFrom(EHealth::composition()->create($compositionRequest)->getData());
    }
