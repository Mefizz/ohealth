...
class MedicationRequestLifecycleService extends EHealthRequestLifecycleService implements EHealthRequestLifecycleContract
{
    ...
    public function rejectPrescription(
        ...
        $needsLocalSign = empty($signedContent) && isset($formData['password'], $formData['knedp']);
        $signerTaxId = $needsLocalSign ? $this->resolveSignerTaxId($formData) : null;
        $activeId = $this->resolveActiveEhealthId($personUuid, (string) $requestRecord->uuid);

        if ($needsLocalSign) {
            // API-005-043-0006: HTTP body is signed blob only; reason lives inside the signed MR.
            $signedContent = signatureService()->signData(
