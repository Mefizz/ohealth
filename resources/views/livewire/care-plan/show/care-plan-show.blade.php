            ...
            <div class="relative pb-2 pr-2">
                ...
                <div x-show="openDropdown" x-transition style="display: none;" class="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none dark:bg-gray-700">
                    <div class="py-1" role="none">
                        <a href="{{ route('care-plans.activities.create', [legalEntity(), $carePlan->id, 'kind' => 'service_request']) }}"
                           @click="openDropdown = false"
                           class="text-gray-700 block w-full px-4 py-2 text-left text-sm hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-600"
                           wire:navigate>
                            {{ __('care-plan.service_prescription') }}
                        </a>
                        <a href="{{ route('care-plans.activities.create', [legalEntity(), $carePlan->id, 'kind' => 'medication_request']) }}"
                           @click="openDropdown = false"
                           class="text-gray-700 block w-full px-4 py-2 text-left text-sm hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-600"
                           wire:navigate>
                            {{ __('care-plan.medication_prescription') }}
                        </a>
                        <a href="{{ route('care-plans.activities.create', [legalEntity(), $carePlan->id, 'kind' => 'device_request']) }}"
                           @click="openDropdown = false"
                           class="text-gray-700 block w-full px-4 py-2 text-left text-sm hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-600"
                           wire:navigate>
                                    ...
                                    <td x-data="{ openDropdown: false }" class="px-4 py-4 text-right relative overflow-visible">
                                        ...
                                        >
                                            @if(in_array(strtoupper($activityStatus), ['NEW', 'DRAFT']))
                                                <div class="py-1">
                                                    <a href="{{ route('care-plans.activities.edit', [legalEntity(), $carePlan->id, $activity->id]) }}"
                                                       @click="openDropdown = false"
