<fieldset class="fieldset">
    ...
    <div x-show="!modalImmunization.vaccineCode" x-cloak>
        <div class="mb-6 flex items-center gap-2 font-semibold text-gray-900 dark:text-gray-100">
            ...
            <span>{{ __('patients.vaccine_search') }}</span>
        </div>

        <div class="form-row-3">
            <div class="form-group group">
                <label for="vaccineSearchName" class="label-modal">
                    {{ __('patients.vaccine_name') }}
                </label>

                <input x-model="vaccineSearch.name"
            ...
            <div class="form-group group">
                <label for="vaccineSearchCode" class="label-modal">
                    {{ __('patients.vaccine_code') }}
                </label>

                <input x-model="vaccineSearch.code"
                       @keydown.enter.prevent="searchVaccines()"
                       type="text"
            ...
            <div class="form-group group">
                <label for="vaccineSearchDisease" class="label-modal">
                    {{ __('patients.disease') }}
                </label>

                <input x-model="vaccineSearch.disease"
                       @keydown.enter.prevent="searchVaccines()"
                       type="text"
        ...
        <div class="mt-6 flex flex-wrap gap-3">
            ...
            <button type="button"
                    @click.prevent="resetVaccineSearch()"
        ...
        {{-- Результати пошуку --}}
        <div x-show="vaccineSearchPerformed"
             x-cloak
        ...
        >
            <table class="table-input w-inherit">
                ...
                <tr>
                    ...
                    <th scope="col" class="th-input text-center">
                        {{ __('forms.action') }}
                    </th>
                </tr>
                </thead>
                ...
                <template x-for="vaccine in vaccineSearchResults" :key="vaccine.code">
                    <tr class="border-b border-gray-200 transition-colors hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800/40">
                        <td class="td-input text-sm text-gray-900 dark:text-white"
                            x-text="vaccine.name"
                        ></td>
                        ...
                        <td class="td-input text-sm text-gray-900 dark:text-white"
                            x-text="
                                vaccine.targetDiseases.length > 0
                                    ? vaccine.targetDiseases
                                        .map((targetDisease) => targetDisease.name)
                                        .join(', ')
                                    : '-'
                            "
                        ></td>
                        ...
                        <td class="td-input text-center">
                            <button type="button"
                                    ...
                                    title="{{ __('patients.select_vaccine') }}"
                            >
                                @icon('plus-circle', 'w-6 h-6')

                                <span class="sr-only">
                                    {{ __('patients.select_vaccine') }}
                                </span>
                            </button>
                        </td>
                    </tr>
                ...
                <template x-if="vaccineSearchResults.length === 0">
                    <tr>
                        <td colspan="4"
                            class="td-input py-8 text-center text-sm text-gray-500 dark:text-gray-400"
                        >
    ...
    {{-- Уже вибрана вакцина --}}
    <div x-show="modalImmunization.vaccineCode"
         x-cloak
         class="grid grid-cols-1 items-end gap-6 lg:grid-cols-[minmax(0,1fr)_auto]"
    >
        <div>
            <div class="label-modal">
                {{ __('patients.vaccine_code_and_name') }} *
            </div>

            <div class="min-h-11 border-b border-gray-300 px-0 py-3 text-base text-gray-900 dark:border-gray-600 dark:text-white"
                 x-text="
                     `${modalImmunization.vaccineCode} - ${
                         vaccineCodesDictionary[modalImmunization.vaccineCode] ?? ''
                     }`
                 "
            ></div>
        </div>
        ...
        @unless($isReadonly)
            ...
            >
                {{ __('patients.choose_another_vaccine') }}
            </button>
        @endunless
    </div>
</fieldset>