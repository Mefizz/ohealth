    ...
    <div class="breadcrumb-form shift-content p-4">
        <div class="mt-6 w-full">
            ...
            @if ($step === Wizard::STEP_ENCOUNTER)
                ...
                @if ($this->needsMother)
                    <div class="mb-6">
                        ...
                        <div class="mb-4 flex max-w-xl flex-wrap items-end gap-2">
                            <div class="form-group group mb-0 min-w-0 flex-1">
                                <input
                                    wire:model="counterpartQuery"
                                    wire:keydown.enter.prevent="searchCounterpart"
                                    type="text"
                                    id="counterpartQuery"
                            ...
                            <button
                                type="button"
                                wire:click="searchCounterpart"
                                class="button-primary flex shrink-0 items-center gap-2 px-4 py-2 text-sm"
                            >
                                @icon('search', 'w-4 h-4')
                        ...
                        <div class="space-y-3">
                            @forelse ($this->counterpartMatches as $match)
                                <div class="record-inner-card" wire:key="mother-{{ $match->id }}">
                                    <div class="record-inner-header items-center gap-3">
                                        <div class="record-inner-column min-w-0 flex-1">
                                        ...
                                        <button
                                            ...
                                            class="button-primary shrink-0 px-3 py-1.5 text-sm whitespace-nowrap"
                                        >
                                            {{ __('forms.select') }}
                                        </button>
                                    </div>
                            ...
                            @empty
                                @if ($counterpartSearchTerm !== '')
                                    <x-nothing-found :description="__('compositions.create_newborn.no_mother')" />
                                @endif
                            @endforelse
                        </div>
                    </div>
                @elseif ($form->personUuid)
                    <div class="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                        <div class="min-w-0">
                        ...
                        <button
                            type="button"
                            wire:click="clearMother"
                            class="button-primary-outline px-4 py-2 text-sm whitespace-nowrap"
                        >
                            {{ __('compositions.create_newborn.change_mother') }}
                        </button>
                    </div>
                @endif
                ...
                @elseif ($form->prepersonUuid && $personId)
                    <div class="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                        <div class="min-w-0">
                            <div class="record-inner-label text-[10px] uppercase">
                                {{ __('compositions.create_newborn.newborn') }}
                        ...
                        <button
                            type="button"
                            wire:click="clearNewborn"
                            class="button-primary-outline px-4 py-2 text-sm whitespace-nowrap"
                        >
                            {{ __('compositions.create_newborn.change_newborn') }}
                        </button>
                    </div>
                @endif
                ...
                @if ($this->needsMother)
                    <p class="mb-4 text-sm text-gray-600 dark:text-gray-300">
                        {{ __('compositions.create_newborn.pick_mother_first') }}
                    </p>
                @elseif ($this->needsNewborn)
                    <p class="mb-4 text-sm text-gray-600 dark:text-gray-300">
                        {{ __('compositions.create_newborn.pick_newborn_first') }}
                    </p>
                @elseif (!$this->needsNewborn && !$this->needsMother)
                        ...
                        @forelse ($this->availableEncounters as $encounter)
                            <div class="record-inner-card" wire:key="encounter-{{ $encounter['uuid'] }}">
                                <div class="record-inner-header">
                                    ...
                                    <div class="flex shrink-0 items-center justify-end border-t border-gray-200 p-3 md:border-t-0 md:border-l dark:border-gray-700">
                                        <button
                                            type="button"
                                            wire:click="selectEncounter('{{ $encounter['uuid'] }}')"
                                            class="button-primary px-3 py-1.5 text-sm whitespace-nowrap"
                                        >
                                            {{ __('forms.select') }}
                                        </button>
                                    </div>
            ...
            @if ($step === Wizard::STEP_AUTH_METHOD)
                ...
                {{-- Warn only when eHealth returned no usable method (TV 3.8.1.4.4). --}}
                @if ($authMethods === [])
            ...
            @if ($step === Wizard::STEP_DETAILS)
                    ...
                    <div class="form-group group">
                        <div class="datepicker-wrapper">
                            <input
                                ...
                                class="datepicker-input with-leading-icon input peer w-full @error('form.newbornBirthDate') input-error @enderror"
                                placeholder=" "
                                autocomplete="off"
                ...
                <div class="flex flex-wrap gap-2">
                    <button type="button" wire:click="submitComposition" class="button-primary px-5 py-2.5 text-sm">
                        {{ __('forms.create') }}
                    </button>
                    <button
                        type="button"
